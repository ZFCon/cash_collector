...
class CashCollectorTests(TestCase):
    def setUp(self):
        ...
        self.task2 = Task.objects.create(
            ...
            amount_due=6000,
            amount_due_at=timezone.now() + timedelta(hours=1),
        )
    ...
    def test_collect_cash(self):
        response = self.client.post(
            "/api/collect/",
            {
                "cash_collector": self.collector.id,
                "task": self.task1.id,
    ...
    def test_frozen_status(self):
        Collection.objects.create(
            cash_collector=self.collector, task=self.task1, amount_collected=1000
        )
        Collection.objects.create(
            cash_collector=self.collector, task=self.task2, amount_collected=6000
        )
    ...
    def test_pay_manager(self):
        ...
        response = self.client.post(
            "/api/pay/", {"cash_collector": self.collector.id, "amount_paid": 500}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.collector.refresh_from_db()
